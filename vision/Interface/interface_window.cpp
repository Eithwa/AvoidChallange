...
#define LAUNCH_PATH "/default_config/AvoidChallenge.yaml"
...
///////////////////////////////BlackItem////////////////////////////////
void interface_window :: Black_Item(cv::Mat &frame_)
{
    //Rate();
    Mat binarization = frame_.clone();
    black_binarization(binarization);
    black_filter(binarization);
    red_binarization(frame_);
    for(int i=0;i<frame_.rows;i++){
        for(int j=0;j<frame_.cols;j++){
            if(frame_.data[(i*frame_.cols+j)*3+2] == 255){
                binarization.data[(i*frame_.cols+j)*3+0] = 0;
                binarization.data[(i*frame_.cols+j)*3+1] = 0;
                binarization.data[(i*frame_.cols+j)*3+2] = 255;
            }
        }
    }
    ///////////////////Show view/////////////////
    ...
    for(int angle = 0; angle < 360; angle = angle + ui->Black_Angle->value()){
        ...
        for(int r = center_inner; r <= center_outer; r++){
            ...
            int image_x = Frame_area(center_x+dis_x,frame_.cols);
            int image_y = Frame_area(center_y-dis_y,frame_.rows);

            if(    binarization.data[(image_y*binarization.cols + image_x)*3+0] == 0
                && binarization.data[(image_y*binarization.cols + image_x)*3+1] == 0
                &&(binarization.data[(image_y*binarization.cols + image_x)*3+2] == 0||binarization.data[(image_y*binarization.cols + image_x)*3+2] == 255)){
                break;
            }else{
                frame_.data[(image_y*frame_.cols + image_x)*3+0] = 0;
                frame_.data[(image_y*frame_.cols + image_x)*3+1] = 0;
                frame_.data[(image_y*frame_.cols + image_x)*3+2] = 255;
            }
        }
    }
    strategy_line(frame_);
...
void interface_window::black_binarization(cv::Mat &frame_){
    for(int i=0;i<frame_.rows;i++){
        for(int j=0;j<frame_.cols;j++){
            unsigned char gray = ( frame_.data[(i*frame_.cols*3)+(j*3)+0]
                                 + frame_.data[(i*frame_.cols*3)+(j*3)+1]
    ...
    ///////////////////Show view/////////////////
    //cv::imshow("black", frame_);
    //cv::waitKey(1);
    /////////////////////////////////////////////
}

void interface_window::black_filter(cv::Mat &Black_Mask){
...
void interface_window::red_binarization(Mat &Red_Mask)
{
    ...
    Mat red(inputMat.rows, inputMat.cols, CV_8UC3, Scalar(0, 0, 255));
    int hmin, hmax, smin, smax, vmin, vmax;
    cvtColor(inputMat, hsv, CV_BGR2HSV);
    hmax = double(HSV_red[0]*0.5);
    hmin = double(HSV_red[1]*0.5);
    smax = HSV_red[2];
    smin = HSV_red[3];
    vmax = HSV_red[4];
    vmin = HSV_red[5];
...
void interface_window::strategy_line(Mat &frame_)
{
    //中心十字線
    line(frame_, Point(center_x,center_y-center_inner), Point(center_x,center_y+center_inner), Scalar(0,255,0), 1);
    line(frame_, Point(center_x-center_inner,center_y), Point(center_x+center_inner,center_y), Scalar(0,255,0), 1);
    ...
    interface->getline();
    //=======遠障礙物規劃================
    double e=interface->far_good_angle;
    e=e*3-270+180;
    line(frame_, Point(center_x,center_y), Point(center_x+center_outer*cos(e*deg2rad),center_y+center_outer*sin(e*deg2rad)), Scalar(18,116,54), 2);
    //=======判斷範圍========
    int f=interface->df_1;
    int g=interface->df_2;
    f=f*3-270+180;
    g=g*3-270+180;
    line(frame_, Point(center_x,center_y), Point(center_x+center_outer*cos(f*deg2rad),center_y+center_outer*sin(f*deg2rad)), Scalar(18,116,54), 8);
    line(frame_, Point(center_x,center_y), Point(center_x+center_outer*cos(g*deg2rad),center_y+center_outer*sin(g*deg2rad)), Scalar(18,116,54), 8);
    //=======================

    //=========近障礙物規劃=============
    double a=interface->good_angle;
    a=a*3-270+180;
    line(frame_, Point(center_x,center_y), Point(center_x+center_outer*cos(a*deg2rad),center_y+center_outer*sin(a*deg2rad)), Scalar(255,0,0), 4);
    //=======判斷範圍========
    int c=interface->dd_1;
    int d=interface->dd_2;
    c=c*3-270+180;
    d=d*3-270+180;
    line(frame_, Point(center_x,center_y), Point(center_x+center_outer*cos(c*deg2rad),center_y+center_outer*sin(c*deg2rad)), Scalar(255,0,0), 1);
    line(frame_, Point(center_x,center_y), Point(center_x+center_outer*cos(d*deg2rad),center_y+center_outer*sin(d*deg2rad)), Scalar(255,0,0), 1);
    //===============================

    //===========合力移動方向==========
    double b=interface->final_angle;
    b=b*3-270+180;
    line(frame_, Point(center_x,center_y), Point(center_x+center_outer*cos(b*deg2rad),center_y+center_outer*sin(b*deg2rad)), Scalar(255,200,0), 6);
