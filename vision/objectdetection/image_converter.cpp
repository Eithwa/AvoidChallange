...
//    FIRA_map = Mat(Size(600,600),CV_8UC3,Scalar(0,0,0));
//    creat_FIRA_map(Main_frame,FIRA_map);
...
void ImageConverter::creat_Obstclemap(Mat &frame_, int color){
  ...
  for(int distance = search_start ; distance <= search_middle ; distance++){
  ...
  cv::dilate(frame_, frame_, Mat(), Point(), 3);
  cv::erode(frame_, frame_, Mat(), Point(), 5);
...
void ImageConverter::creat_FIRA_map(Mat &frame_input , Mat &frame_output){
  ...
  int FIRA_x,FIRA_y,FIRA_x_lo,FIRA_y_lo;
  ...
  for(int angle = 0; angle < 360; angle++){
    for(int distance = search_start ; distance < search_end ; distance ++){
      x = distance*Angle_cos[angle];
      y = distance*Angle_sin[angle];
      x_image = Frame_area(center_x+x,frame_input.cols);
      y_image = Frame_area(center_y-y,frame_input.rows);

      for(i = 0; i < 2;i++){
        for(j = 0; j < 2 ; j++){
          FIRA_x = Omni_distance(Frame_area(distance+j,search_end))*Angle_cos[Planning_angle(angle+i,360)]/2;
          FIRA_y = Omni_distance(Frame_area(distance+j,search_end))*Angle_sin[Planning_angle(angle+i,360)]/2;

          FIRA_x_lo = Frame_area(frame_center_x+FIRA_x,frame_output.cols);
          FIRA_y_lo = Frame_area(frame_center_y-FIRA_y,frame_output.rows);

          points[0][i*2+j] = Point(FIRA_x_lo,FIRA_y_lo);
        }
      }
      unsigned char B = frame_input.data[(y_image*frame_input.cols+x_image)*3+0];
      unsigned char G = frame_input.data[(y_image*frame_input.cols+x_image)*3+1];
      unsigned char R = frame_input.data[(y_image*frame_input.cols+x_image)*3+2];

      Point points_[1][4];

      points_[0][0] = points[0][0];
      points_[0][1] = points[0][1];
      points_[0][2] = points[0][3];
      points_[0][3] = points[0][2];

      const Point* ppt[1] = {points_[0]};
      int npt[] = {4};
      fillPoly(frame_output, ppt, npt, 1, Scalar(B,G,R),1);
    }
  }
...
void ImageConverter::draw_FIRA_map(Mat &frame_, object_Item &obj_item){
  int frame_center_x = frame_.cols / 2;
  int frame_center_y = frame_.rows / 2;
  ...
  Point *points = new Point [obj_item.ang_max - obj_item.ang_min];

  int distance_get = 0;
  int point_num = 0;

  for(int angle = obj_item.ang_min; angle < obj_item.ang_max; angle++){
    for(int distance = obj_item.dis_min ; distance <= obj_item.dis_max ; distance++){
      x_= distance*Angle_cos[angle];
      y_= distance*Angle_sin[angle];

      x = Frame_area(center_x+x_,Obstaclemap.cols);
      y = Frame_area(center_y-y_,Obstaclemap.rows);

      if(Obstaclemap.data[(y*Obstaclemap.cols+x)*3+0]==255){
        ...
        x = Frame_area(frame_center_x+x_,frame_.cols);
        y = Frame_area(frame_center_y-y_,frame_.rows);

        points[point_num++] = Point(x, y);
        break;
      }
    }
    if(distance_get == 0){
      dis = Omni_distance(obj_item.dis_min);
  ...
  for(int i=0 ;i<point_num-1;i++){
    line(frame_, points[i], points[i+1], 255, 1);
  }
...
//  find_object_point(*(obj_item+4),color);
  ...
  draw_FIRA_map(FIRA_map,Obstacle_Item[0]);
  draw_FIRA_map(FIRA_map,Obstacle_Item[1]);
  draw_FIRA_map(FIRA_map,Obstacle_Item[2]);
  draw_FIRA_map(FIRA_map,Obstacle_Item[3]);
  draw_FIRA_map(FIRA_map,Obstacle_Item[4]);
...
double ImageConverter::Omni_distance(double dis_pixel){
  ...
  double pixel_dis = abs(dis_pixel);
  ...
  if(dis_pixel<0) return (dis/10)*-1;
  else  return dis/10;
}
